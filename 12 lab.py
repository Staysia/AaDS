"""23. Вычислить сумму знакопеременного ряда |х*2n!|/(2n)!,
где х-матрица ранга к (к и матрица задаются случайным образом),
n - номер слагаемого. Сумма считается вычисленной, если точность
вычислений будет не меньше t знаков после запятой.
...
factorial_n = math.factorial(n) # факториал числителя
factorial_2n = math.factorial(2 * n) # факториал знаменателя
znak = 1 # знак
...
while abs(rezultat.as_tuple().exponent) < t: # цикл while пока число знаков после запятой не будет больше или равно t
    fun = de.Decimal(np.linalg.det(matic * 2 *  factorial_n)) * de.Decimal((1/factorial_2n)) # вычисление слагаемого
    rezultat = rezultat + (znak * fun) # прибавление результата с учётом знака
    n += 1 # следующий номер слагаемого
    factorial_n *= n # новый факториал с учётом нового номера слагаемого
    factorial_2n *= (2 * n) # новый факториал с учётом нового номера слагаемого
